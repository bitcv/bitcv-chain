...
func (gs *Store) Get(key []byte) (value []byte) {
	...
	gs.gasMeter.ConsumeGas(gs.gasConfig.ReadCostPerByte*types.Gas(getNewLen(len(value))), types.GasReadPerByteDesc)
...
func (gs *Store) Set(key []byte, value []byte) {
	...
	// TODO overflow-safe math?
	gs.gasMeter.ConsumeGas(gs.gasConfig.WriteCostPerByte*types.Gas(getNewLen(len(value))), types.GasWritePerByteDesc)
...
10 byte 1gas
 */
func getNewLen(len int)  int {
	a := math.Ceil(float64(len) / 10.0)
	return int(a)
}