...
sim() {
  seed=$1
	echo "Running Bac simulation with seed $seed. This may take awhile!"
  file="$tmpdir/bac-simulation-seed-$seed-date-$(date -u +"%Y-%m-%dT%H:%M:%S+00:00").stdout"
  echo "Writing stdout to $file..."
	go test ./cmd/bac/app -run $testname -SimulationEnabled=true -SimulationNumBlocks=$blocks -SimulationGenesis=$genesis \
...
for pid in ${pids[*]}; do
  ...
  then
    ...
    echo "To replicate, run 'go test ./cmd/bac/app -run $testname -SimulationEnabled=true -SimulationNumBlocks=$blocks -SimulationVerbose=true -SimulationCommit=true -SimulationSeed=$seed -v -timeout 24h'"
    code=1
  else
