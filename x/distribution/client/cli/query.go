...
import (
	...
	"github.com/bitcv-chain/bitcv-chain/x/distribution/client/common"
...
func GetCmdQueryValidatorOutstandingRewards(queryRoute string, cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			params := distr.NewQueryValidatorCommissionParams(valAddr)
...
$ baccli query distr commission bacvaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
...
$ baccli query distr slashes bacvaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj 0 100
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
...
$ baccli query distr rewards bac1gghjut3ccd8ay0zduzj64hwre2fxs9ld75ru9p
$ baccli query distr rewards bac1gghjut3ccd8ay0zduzj64hwre2fxs9ld75ru9p bacvaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
			...
			if len(args) == 2 {
				...
				var result types.RewardsDetailInfo
				...
				var result types.TotalRewardsDetailInfo
				cdc.MustUnmarshalJSON(resp, &result)
				return cliCtx.PrintOutput(result)
			}
...
$ baccli query distr community-pool
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
