...
import (
	...
	"github.com/bitcv-chain/bitcv-chain/x/distribution"
...
func validatorInfoHandlerFn(cliCtx context.CLIContext, cdc *codec.Codec,
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var rewardsDetailInfo types.RewardsDetailInfo
		cdc.MustUnmarshalJSON(rewardsRes, &rewardsDetailInfo)

		// Prepare response
		res := cdc.MustMarshalJSON(NewValidatorDistInfo(delAddr, rewardsDetailInfo.Rewards, valCom))
		rest.PostProcessResponse(w, cdc, res, cliCtx.Indent)
	}
}
