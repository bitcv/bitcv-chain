...
func (k Keeper) withdrawDelegationRewards(ctx sdk.Context, val sdk.Validator, del sdk.Delegation) (sdk.Coins,sdk.Coins,sdk.Error) {
	// check existence of delegator starting info
	if !k.HasDelegatorStartingInfo(ctx, del.GetValidatorAddr(), del.GetDelegatorAddr()) {
		return nil,sdk.Coins{}, types.ErrNoDelegationDistInfo(k.codespace)
	}
	...
	//startingInfo := k.GetDelegatorStartingInfo(ctx, del.GetValidatorAddr(), del.GetDelegatorAddr())
	/* 判断能量*/
	consumeEnergy ,startingInfo:= k.GetMinerCostEnergyAndStartingInfo(ctx,del)
	consumeEnergyCoins := sdk.NewCoins(sdk.NewCoin(sdk.CHAIN_COIN_NAME_ENERGY, consumeEnergy))
	_,err := k.bankKeeper.SendCoins(ctx, del.GetDelegatorAddr(),sdk.AccAddrEnergyBurn,consumeEnergyCoins)
	diffHeight := ctx.BlockHeight() -  int64(startingInfo.Height)
	ctx.Logger().Debug("module", "startingInfo", startingInfo,"diffHeight",diffHeight,"consumeEnergy",consumeEnergy)
	if err != nil   {
		ctx.Logger().Debug("module", "energy is not enough", err, "consumeEnergy", consumeEnergy)
	...
	if !coins.IsZero() {
		...
		if _, _, err := k.bankKeeper.AddCoins(ctx, withdrawAddr, coins); err != nil {
			return nil,consumeEnergyCoins, err
		}
	}
	...
	return coins,consumeEnergyCoins, nil
}


//计算挖矿需要消耗的能量,返回耗费能量和开始抵押时间
func (k Keeper) GetMinerCostEnergyAndStartingInfo(ctx sdk.Context, del sdk.Delegation) (sdk.Int,types.DelegatorStartingInfo){
	startingInfo := k.GetDelegatorStartingInfo(ctx,del.GetValidatorAddr(),del.GetDelegatorAddr())
	diffHeight := ctx.BlockHeight() -  int64(startingInfo.Height)
	amt := startingInfo.Stake // Get bcv amount, one bcv one energy
	consumeEnergy := sdk.GetConsumeEnergy(amt, diffHeight)
	return consumeEnergy,startingInfo
}


//计算挖矿需要消耗的能量,只返回耗费能量
func (k Keeper) GetMinerCostEnergy(ctx sdk.Context, del sdk.Delegation) (sdk.Int){
	startingInfo := k.GetDelegatorStartingInfo(ctx,del.GetValidatorAddr(),del.GetDelegatorAddr())
	diffHeight := ctx.BlockHeight() -  int64(startingInfo.Height)
	amt := startingInfo.Stake // Get bcv amount, one bcv one energy
	consumeEnergy := sdk.GetConsumeEnergy(amt, diffHeight)
