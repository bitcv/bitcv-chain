...
func queryDelegationRewards(ctx sdk.Context, _ []string, req abci.RequestQuery, k Keeper) ([]byte, sdk.Error) {
	...
	costEnergy , startingInfo := k.GetMinerCostEnergyAndStartingInfo(ctx,del)
	rewardsDetailInfo  := types.RewardsDetailInfo{
		OperAddr:params.ValidatorAddress.String(),
	...
	bz, err := codec.MarshalJSONIndent(k.cdc, rewardsDetailInfo)
	if err != nil {
		return nil, sdk.ErrInternal(sdk.AppendMsgToErr("could not marshal result to JSON", err.Error()))
	}
...
func queryDelegatorTotalRewards(ctx sdk.Context, _ []string, req abci.RequestQuery, k Keeper) ([]byte, sdk.Error) {
	...
	var totalRewardsDetail types.TotalRewardsDetailInfo
	k.stakingKeeper.IterateDelegations(
		...
		func(_ int64, del sdk.Delegation) (stop bool) {
			...
			costEnergy , startingInfo := k.GetMinerCostEnergyAndStartingInfo(ctx,del)
			rewardsDetailInfo  := types.RewardsDetailInfo{
				OperAddr:del.GetValidatorAddr().String(),
				StartingInfo:startingInfo,
				Rewards:rewards,
				CostEnergy:costEnergy,
				EndHeight:ctx.BlockHeight(),
			}
			totalRewardsDetail.RewardList = append(totalRewardsDetail.RewardList,rewardsDetailInfo )
			totalRewards = totalRewards.Add(rewards)
			return false
		},
	)
	totalRewardsDetail.TotalRewards = totalRewards
	bz, err := codec.MarshalJSONIndent(k.cdc, totalRewardsDetail)
	if err != nil {
		return nil, sdk.ErrInternal(sdk.AppendMsgToErr("could not marshal result to JSON", err.Error()))
	}
