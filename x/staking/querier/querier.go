...
import (
	...
	keep "github.com/bitcv-chain/bitcv-chain/x/staking/keeper"
...
const (
	...
	QueryDelegationsAll				   =  "delegationsAll"
)
...
func NewQuerier(k keep.Keeper, cdc *codec.Codec) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) (res []byte, err sdk.Error) {
		...
		case QueryDelegationsAll:
			return queryDelegationsAll(ctx, cdc, req, k);
		default:
...
func queryValidators(ctx sdk.Context, cdc *codec.Codec, req abci.RequestQuery, k keep.Keeper) ([]byte, sdk.Error) {
	...
	if  "all" ==strings.ToLower(params.Status) {
	...
	//添加ValidatorForApi，方便客户端解析验证节点地址
	filteredValApis := make([]types.ValidatorForApi, 0, len(filteredVals))
	for _, val := range  filteredVals {
		tmp :=  types.ValidatorForApi{
			Validator:val,
			AddrBench32:sdk.AccAddress(val.OperatorAddress).String(),
			AddrFromPubkey:val.ConsPubKey.Address(),
		}
		filteredValApis  = append(filteredValApis,tmp)
	}

	res, err := codec.MarshalJSONIndent(cdc, filteredValApis)
	if err != nil {
...
func queryDelegationsAll(ctx sdk.Context, cdc *codec.Codec, req abci.RequestQuery, k keep.Keeper) ([]byte, sdk.Error) {
	var params QueryValidatorsParams
