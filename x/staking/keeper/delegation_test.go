...
func TestUnbondDelegation(t *testing.T) {
	...
	amount,_,err := keeper.unbond(ctx, addrDels[0], addrVals[0], bondTokens.ToDec())
...
func TestUnbondingDelegationsMaxEntries(t *testing.T) {
	...
	for i := uint16(0); i < maxEntries; i++ {
		var err error
		completionTime,_, err = keeper.Undelegate(ctx, addrDels[0], addrVals[0], sdk.NewDec(1))
		require.NoError(t, err)
	}

	// an additional unbond should fail due to max entries
	_,_, err := keeper.Undelegate(ctx, addrDels[0], addrVals[0], sdk.NewDec(1))
	require.Error(t, err)
...
func TestUndelegateSelfDelegationBelowMinSelfDelegation(t *testing.T) {
	...
	_,_,err := keeper.Undelegate(ctx, val0AccAddr, addrVals[0], sdk.TokensFromTendermintPower(6).ToDec())
	require.NoError(t, err)
...
func TestUndelegateFromUnbondingValidator(t *testing.T) {
	...
	val0AccAddr := sdk.AccAddress(addrVals[0].Bytes())
	_,_,err := keeper.Undelegate(ctx, val0AccAddr, addrVals[0], valTokens.ToDec())
	require.NoError(t, err)
	...
	_,_,err = keeper.Undelegate(ctx, addrDels[0], addrVals[0], sdk.NewDec(6))
	require.NoError(t, err)
...
func TestUndelegateFromUnbondedValidator(t *testing.T) {
	...
	_,_,err = keeper.Undelegate(ctx, addrDels[0], addrVals[0], unbondTokens.ToDec())
	require.NoError(t, err)

	// unbond rest of the other delegation's shares
	remainingTokens := delTokens.Sub(unbondTokens)
	_,_,err = keeper.Undelegate(ctx, addrDels[0], addrVals[0], remainingTokens.ToDec())
	require.NoError(t, err)
...
func TestUnbondingAllDelegationFromValidator(t *testing.T) {
	...
	_, _,err = keeper.Undelegate(ctx, addrDels[0], addrVals[0], delTokens.ToDec())
	require.NoError(t, err)
...
func TestRedelegateToSameValidator(t *testing.T) {
	...
	_, _,err := keeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[0], sdk.NewDec(5))
	require.Error(t, err)
...
func TestRedelegationMaxEntries(t *testing.T) {
	...
	for i := uint16(0); i < maxEntries; i++ {
		var err error
		completionTime,_, err = keeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[1], sdk.NewDec(1))
		require.NoError(t, err)
	}

	// an additional redelegation should fail due to max entries
	_, _,err := keeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[1], sdk.NewDec(1))
	require.Error(t, err)
...
func TestRedelegateSelfDelegation(t *testing.T) {
	...
	_, _,err := keeper.BeginRedelegation(ctx, val0AccAddr, addrVals[0], addrVals[1], delTokens.ToDec())
	require.NoError(t, err)
...
func TestRedelegateFromUnbondingValidator(t *testing.T) {
	...
	// unbond the all self-delegation to put validator in unbonding state
	_, _,err := keeper.Undelegate(ctx, val0AccAddr, addrVals[0], delTokens.ToDec())
	require.NoError(t, err)
	...
	_, _,err = keeper.BeginRedelegation(ctx, addrDels[0], addrVals[0], addrVals[1], redelegateTokens.ToDec())
	require.NoError(t, err)
...
func TestRedelegateFromUnbondedValidator(t *testing.T) {
	...
	_, _,err = keeper.BeginRedelegation(ctx, addrDels[0], addrVals[0], addrVals[1], redelegationTokens.ToDec())
	require.NoError(t, err)
