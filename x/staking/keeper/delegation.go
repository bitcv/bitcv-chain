...
func (k Keeper) Delegate(ctx sdk.Context, delAddr sdk.AccAddress, bondAmt sdk.Int,
	...
	if found {
		//代理已经存在,需要先调用提现,提现需要消耗能量,如果能量不足直接返回
		costEnergy := k.distrKeeper.GetMinerCostEnergy(ctx,delegation)
		isHaveCoin := k.bankKeeper.HasCoins(ctx,delAddr,sdk.NewCoins(sdk.NewCoin(sdk.CHAIN_COIN_NAME_ENERGY,costEnergy)))
		if !isHaveCoin{
			return sdk.Dec{},types.ErrNotEnoughEnergy(k.Codespace(),costEnergy)
		}
...
func (k Keeper) unbond(ctx sdk.Context, delAddr sdk.AccAddress, valAddr sdk.ValAddress,
	shares sdk.Dec) (amount sdk.Int,costEnergyCoins sdk.Coins, err sdk.Error) {

	// check if a delegation object exists in the store
	delegation, found := k.GetDelegation(ctx, delAddr, valAddr)
	if !found {
		return amount,sdk.Coins{}, types.ErrNoDelegatorForAddress(k.Codespace())
	}
	//消耗的能量,如果能量不足直接返回
	costEnergy := k.distrKeeper.GetMinerCostEnergy(ctx,delegation)
	costEnergyCoins = sdk.NewCoins(sdk.NewCoin(sdk.CHAIN_COIN_NAME_ENERGY,costEnergy))
	isHaveCoin := k.bankKeeper.HasCoins(ctx,delAddr,costEnergyCoins)
	if !isHaveCoin{
		return amount,costEnergyCoins,types.ErrNotEnoughEnergy(k.Codespace(),costEnergy)
	}
	...
	if delegation.Shares.LT(shares) {
		return amount,costEnergyCoins, types.ErrNotEnoughDelegationShares(k.Codespace(), delegation.Shares.String())
	}

	// get validator
	validator, found := k.GetValidator(ctx, valAddr)
	if !found {
		return amount,costEnergyCoins, types.ErrNoValidatorFound(k.Codespace())
	}
	...
	return amount, costEnergyCoins,nil
}
...
func (k Keeper) Undelegate(
...
) (time.Time, sdk.Coins,sdk.Error) {

	returnAmount,costEnergyCoins, err := k.unbond(ctx, delAddr, valAddr, sharesAmount)
	if err != nil {
		return time.Time{},costEnergyCoins, err
	}

	if k.HasMaxUnbondingDelegationEntries(ctx, delAddr, valAddr) {
		return time.Time{}, costEnergyCoins,types.ErrMaxUnbondingDelegationEntries(k.Codespace())
	}
	...
	return completionTime, costEnergyCoins,nil
}
	...
	for i := 0; i < len(ubd.Entries); i++ {
		...
		if entry.IsMature(ctxTime) {
			...
			if !entry.Balance.IsZero() {
				...
				//得到ubcvstake之后,直接1:1兑换成ubcv;只有正常运行状态的验证人可以做这样的操作
				err = k.ExchangeBcvstakeToBcv(ctx,ubd.DelegatorAddress,entry.Balance)
				if err != nil {
					return err
				}
...
func (k Keeper) BeginRedelegation(ctx sdk.Context, delAddr sdk.AccAddress,
	...
	completionTime time.Time, costEnergyCoins sdk.Coins,errSdk sdk.Error) {

	if bytes.Equal(valSrcAddr, valDstAddr) {
		return time.Time{},sdk.Coins{}, types.ErrSelfRedelegation(k.Codespace())
	}

	// check if this is a transitive redelegation
	if k.HasReceivingRedelegation(ctx, delAddr, valSrcAddr) {
		return time.Time{},sdk.Coins{}, types.ErrTransitiveRedelegation(k.Codespace())
	}

	if k.HasMaxRedelegationEntries(ctx, delAddr, valSrcAddr, valDstAddr) {
		return time.Time{},sdk.Coins{}, types.ErrMaxRedelegationEntries(k.Codespace())
	}

	returnAmount,costEnergyCoins, err := k.unbond(ctx, delAddr, valSrcAddr, sharesAmount)
	if err != nil {
		return time.Time{}, costEnergyCoins,err
	}

	if returnAmount.IsZero() {
		return time.Time{},costEnergyCoins, types.ErrVerySmallRedelegation(k.Codespace())
	}
	dstValidator, found := k.GetValidator(ctx, valDstAddr)
	if !found {
		return time.Time{}, costEnergyCoins,types.ErrBadRedelegationDst(k.Codespace())
	}
 ...
 *验证人将bcvstake 兑换成bcv
  *目前是1:1
 */
func (k Keeper) ExchangeBcvstakeToBcv(ctx sdk.Context, accAddr sdk.AccAddress,  amt sdk.Int) (err sdk.Error) {

	_, found := k.GetValidator(ctx, sdk.ValAddress(accAddr))
	if !found {
		return  types.ErrNoValidatorFound(k.Codespace())
	}

	//向bcvstakeaddr归还bitcvstake
	bcvstakeCoin := sdk.NewCoins(sdk.NewCoin(sdk.DefaultBondDenom,amt))
	_, err = k.bankKeeper.SendCoins(ctx,accAddr,sdk.AccAddrBcvstakePool,bcvstakeCoin);
	if err != nil {
		return err
	}

	//获取BCV
	bcvCoin := sdk.NewCoins(sdk.NewCoin(sdk.DefaultBCVDemon,amt))
	_, err = k.bankKeeper.SendCoins(ctx,sdk.AccAddrBcvstakePool,accAddr,bcvCoin);
	if err != nil {
		return err
	}
	return nil
 ...
 *liuhaoyang
 */
func (k Keeper) BurnBcvToEnergy(ctx sdk.Context, accAddr sdk.AccAddress,  amt sdk.Int) (err sdk.Error,reciveEnergy sdk.Coins) {

	energyAmt := k.GetEnergyFromBcv(amt)
	energyAmtCoins := sdk.NewCoins(sdk.NewCoin(sdk.CHAIN_COIN_NAME_ENERGY,energyAmt))
	_,err  = k.bankKeeper.SendCoins(ctx,sdk.AccAddrEnergyPool,accAddr,energyAmtCoins)
	if err != nil {
		return err,sdk.Coins{}
	}


	bcvCoin := sdk.NewCoins(sdk.NewCoin(sdk.DefaultBCVDemon,amt))
	_, err = k.bankKeeper.SendCoins(ctx,accAddr,sdk.AccAddrEnergyPool,bcvCoin);
	if err != nil {
		return err,sdk.Coins{}
	}
	return nil,energyAmtCoins
 ...
 *liuhaoyang
 */
func (k Keeper)BurnBcvToBcvstake(ctx sdk.Context, accAddr sdk.AccAddress,  amt sdk.Int) (err sdk.Error,reciveStakeAmount sdk.Coins) {
	bcvstakeAmt := k.GetStakeFromBcv(amt)
	bcvStakeCoins := sdk.NewCoins(sdk.NewCoin(sdk.DefaultBondDenom,bcvstakeAmt))
	_, err = k.bankKeeper.SendCoins(ctx,sdk.AccAddrBcvstakePool,accAddr,bcvStakeCoins);
	if err != nil {
		return err,sdk.Coins{}
	}

	bcvCoin := sdk.NewCoins(sdk.NewCoin(sdk.DefaultBCVDemon,amt))
	_, err = k.bankKeeper.SendCoins(ctx,accAddr,sdk.AccAddrBcvBurnFromBuyBcvStake,bcvCoin);
	if err != nil {
		return err,sdk.Coins{}
	}
	return nil,bcvStakeCoins
}

//acc energy from ubcv
func (k Keeper) GetEnergyFromBcv(amt sdk.Int) (sdk.Int) {
	rate,_:= sdk.NewDecFromStr(sdk.RateBurnBcvToEnergy)
	energyAmt := amt.ToDec().Mul(rate).TruncateInt()
	return energyAmt
}

//acc ubcvstake from ubcv
func (k Keeper) GetStakeFromBcv(amt sdk.Int) (sdk.Int) {
	rate,_:= sdk.NewDecFromStr(sdk.RateBurnBcvToBcvstake)
	energyAmt := amt.ToDec().Mul(rate).TruncateInt()
	return energyAmt
}
 ...
 * liuhaoyang
 */
func (k Keeper) ValidatorPledgeBcvToBcvstake(ctx sdk.Context, delAddr sdk.AccAddress,  amt sdk.Int) (err sdk.Error) {

	bcvShareCoin := sdk.NewCoins(sdk.NewCoin(sdk.DefaultBondDenom,amt))
	_, err = k.bankKeeper.SendCoins(ctx,sdk.AccAddrBcvstakePool,delAddr,bcvShareCoin);
	if err != nil {
		return err
	}

	bcvCoin := sdk.NewCoins(sdk.NewCoin(sdk.DefaultBCVDemon,amt))
	_, err = k.bankKeeper.SendCoins(ctx,delAddr,sdk.AccAddrBcvstakePool,bcvCoin);
	if err != nil {
		return err
	}
	return nil
