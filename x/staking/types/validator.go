...
type Validator struct {
	...
	CanUnbondToken		sdk.Int			`json:"can_unbond_token"`  //创建时绑定的token数量
}
...
func (v Validator) String() string {
  ...
  Commission:                 %s
  CanUnbondToken:           %s`, v.OperatorAddress, bechConsPubKey,
		v.Jailed, v.Status, v.Tokens,
		v.DelegatorShares, v.Description,
		v.UnbondingHeight, v.UnbondingCompletionTime, v.MinSelfDelegation, v.Commission,v.CanUnbondToken)
}
...
type bechValidator struct {
	...
	CanUnbondToken		sdk.Int			`json:"can_unbond_token"`
}
...
func (v Validator) MarshalJSON() ([]byte, error) {
	...
	return codec.Cdc.MarshalJSON(bechValidator{
		...
		CanUnbondToken:		v.CanUnbondToken,
	})
}
...
func (v *Validator) UnmarshalJSON(data []byte) error {
	...
	*v = Validator{
		...
		CanUnbondToken:        bv.CanUnbondToken,
	}
	return nil
}
...
func (v Validator) SetCanUnbondToken(amount sdk.Int) Validator {
	v.CanUnbondToken = amount
	return  v
}

func (v Validator) SubCanUnbondToken(amount sdk.Int) Validator {
	v.CanUnbondToken = v.CanUnbondToken.Sub(amount)
	if v.CanUnbondToken.IsNegative(){
		panic("created bond token is negative")
	}
	return  v
}
...
/**
	api调用
 */
type ValidatorForApi struct {
	Validator Validator
	AddrFromPubkey		crypto.Address		`json:"addr_from_pubkey"` //pubkey转化
	AddrBench32			string				`json:"addr_bench32"`
}