...
$ baccli tx staking delegate bacvaloper1l2rsakp388kuv9k8qzq6lrm9taddae7fpx59wm 1000stake --from mykey
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			txBldr := authtxb.NewTxBuilderFromCLI().WithTxEncoder(auth.DefaultTxEncoder(cdc))
			cliCtx := context.NewCLIContext().
...
$ baccli tx staking redelegate bacvaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj bacvaloper1l2rsakp388kuv9k8qzq6lrm9taddae7fpx59wm 100stake --from mykey
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			txBldr := authtxb.NewTxBuilderFromCLI().WithTxEncoder(auth.DefaultTxEncoder(cdc))
			cliCtx := context.NewCLIContext().
...
$ baccli tx staking unbond bacvaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj 100stake --from mykey
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			txBldr := authtxb.NewTxBuilderFromCLI().WithTxEncoder(auth.DefaultTxEncoder(cdc))
			cliCtx := context.NewCLIContext().
...
func GetCmdExchangeBcvstakeToBcv(storeName string, cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "exchange-bcvstake-to-bcv [amount]",
		Short: "exchange bcvstake to bcv",
		Args:  cobra.ExactArgs(1),
		Long: strings.TrimSpace(`Unbond an amount of bonded shares from a validator:

$ baccli tx staking bcvstake bacvaloper1gghjut3ccd8ay0zduzj64hwre2fxs9ldmqhffj 100stake --from mykey
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			txBldr := authtxb.NewTxBuilderFromCLI().WithTxEncoder(auth.DefaultTxEncoder(cdc))
			cliCtx := context.NewCLIContext().
			...
			if amount.Denom != sdk.DefaultBondDenom{
				fmt.Errorf("coin denom should be ubcvshare error get : %s", amount)
			}

			msg := staking.NewMsgExchangeBcvstakeToBcv(accAddr, amount)
			return utils.GenerateOrBroadcastMsgs(cliCtx, txBldr, []sdk.Msg{msg}, false)
		},
	}
}
...
func GetCmdBurnBcvToEnergy(storeName string, cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "burn-bcv-to-energy [amount]",
		Short: "burn bcv to energy",
		Args:  cobra.ExactArgs(1),
		Long: strings.TrimSpace(`Unbond an amount of bonded shares from a validator:
...
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := staking.NewMsgBurnBcvToEnergy(accAddr, amount)
			return utils.GenerateOrBroadcastMsgs(cliCtx, txBldr, []sdk.Msg{msg}, false)
		},
	}
}
...
func GetCmdBurnBcvToBcvstake(storeName string, cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "burn-bcv-to-bcvstake [amount]",
		Short: "burn bcvstake to bcv",
		Args:  cobra.ExactArgs(1),
		Long: strings.TrimSpace(`Unbond an amount of bonded shares from a validator:
...
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := staking.NewMsgBurnBcvToBcvstake(accAddr, amount)
			return utils.GenerateOrBroadcastMsgs(cliCtx, txBldr, []sdk.Msg{msg}, false)
		},
	}
}