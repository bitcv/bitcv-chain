...
func registerQueryRoutes(cliCtx context.CLIContext, r *mux.Router, cdc *codec.Codec) {
	...
	// Get all validators
	r.HandleFunc(
		"/staking/delegations/all",
		delegationsAllHandlerFn(cliCtx, cdc),
	).Methods("GET")
}
...
// HTTP request handler to query list of validators
func delegationsAllHandlerFn(cliCtx context.CLIContext, cdc *codec.Codec) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		_, page, limit, err := rest.ParseHTTPArgs(r)
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusBadRequest, err.Error())
		...
		route := fmt.Sprintf("custom/%s/%s", staking.QuerierRoute, staking.QueryDelegationsAll)
		res, err := cliCtx.QueryWithData(route, bz)
		if err != nil {
			rest.WriteErrorResponse(w, http.StatusInternalServerError, err.Error())
			return
