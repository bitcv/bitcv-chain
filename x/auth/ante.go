...
const (
	HeightHalfStopBlock = int64(9710029) // 2 year
)
...
func NewAnteHandler(ak AccountKeeper, fck FeeCollectionKeeper) sdk.AnteHandler {
	return func(
		...
		if !stdTx.Fee.Amount.IsZero() {
			...
			//目前销毁手续费//
			fck.BurnSupply(newCtx,stdTx.Fee.Amount)
		}
...
func EnsureSufficientMempoolFees(ctx sdk.Context, stdFee StdFee) sdk.Result {
	...
	if !minGasPrices.IsZero() {
		...
		//stdFee.Amount[0].IsLT(sdk.NewCoin(sdk.DEFAULT_FEE_COIN,sdk.NewInt(1500000)
		if len(stdFee.Amount) != 1 || stdFee.Amount[0].Denom != sdk.DEFAULT_FEE_COIN{
			return sdk.ErrInsufficientFee("need more fees").Result()
		}
		...
		//if no fees，err
		return sdk.ErrInsufficientFee("need set min gas prices first").Result()
	}
