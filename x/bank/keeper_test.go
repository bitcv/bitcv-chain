...
func setupTestInput() testInput {
	...
	return testInput{cdc: cdc, ctx: ctx, ak: ak, pk: pk,ak_cdc:cdc,ak_storeKey:authCapKey}
}
...
func TestIssueTokenKeeper(t *testing.T) {
	var err sdk.Error
	input := setupTestInput()
	ctx := input.ctx
	paramSpace := input.pk.Subspace(DefaultParamspace)
	bankKeeper := NewBaseKeeper(input.ak, paramSpace, DefaultCodespace)
	issueTokenKeeper := NewIssueTokenKeeper(input.ak_storeKey,input.ak_cdc,DefaultCodespace)
	bankKeeper.itk = issueTokenKeeper
	...
	input.ak.SetAccount(ctx, acc)
	require.True(t, sendKeeper.GetCoins(ctx, addr1).IsEqual(sdk.NewCoins()))

	//新币
	issueToken := NewIssueToken(addr1,"test1", sdk.NewInt(100000000),sdk.Coin{Denom:"ubcv",Amount:sdk.NewInt(1000000000)},
		uint8(2),"http://www,bitcv.com","http://www,bitcv.com")
	_,err,_ = bankKeeper.IssueToken(ctx,issueToken)
	require.Equal(t,sdk.CodeInsufficientCoins,sdk.ErrInsufficientCoins("").Code())

	//发币
	var addBcvCoin =sdk.Coin{Denom:"ubcv",Amount:sdk.NewInt(1000000000)}
	bankKeeper.AddCoins(ctx,addr1,sdk.Coins{addBcvCoin})
	_,err ,saveIssueToken:= bankKeeper.IssueToken(ctx,issueToken)
	require.Equal(t,err,nil)
	require.Equal(t,bankKeeper.GetCoins(ctx,addr1).AmountOf("ubcv"),sdk.ZeroInt())
	require.Equal(t,bankKeeper.GetCoins(ctx,saveIssueToken.ExchangeAddress).IsAnyGTE(sdk.Coins{addBcvCoin}),true)
	require.Equal(t,saveIssueToken.ExchangeRate,sdk.MustNewDecFromStr("0.1"))
	//转账
	innserName := saveIssueToken.InnerName
	bankKeeper.SendCoins(ctx,addr1,addr2,sdk.Coins{sdk.Coin{Denom:innserName,Amount:sdk.NewInt(5000000000)}})
	require.Equal(t,bankKeeper.GetCoins(ctx,addr1).AmountOf(innserName),sdk.NewInt(5000000000))
	require.Equal(t,bankKeeper.GetCoins(ctx,addr2).AmountOf(innserName),sdk.NewInt(5000000000))

	//赎回
	redeemCoin  := sdk.Coin{Denom:innserName,Amount:sdk.NewInt(1)}
	_,err = bankKeeper.Redeem(ctx,addr2,redeemCoin)
	require.Equal(t,err,nil)
	redeemCoin  = sdk.Coin{Denom:innserName,Amount:sdk.NewInt(4000000000)}
	_,err = bankKeeper.Redeem(ctx,addr2,redeemCoin)
	fmt.Println(bankKeeper.GetCoins(ctx,saveIssueToken.ExchangeAddress))
	require.Equal(t,bankKeeper.GetCoins(ctx,saveIssueToken.ExchangeAddress).AmountOf("ubcv"),sdk.NewInt(600000000))

	//增加抵押金
	addBcvCoin =sdk.Coin{Denom:"ubcv",Amount:sdk.NewInt(1000000000)}
	bankKeeper.AddCoins(ctx,addr1,sdk.Coins{addBcvCoin})
	_,_,newSaveToken := bankKeeper.AddMarginByInnerName(ctx,addr1,innserName,addBcvCoin)
	fmt.Println(newSaveToken.ExchangeRate)
