...
const (
	...
	DefaultMintDenom = "nbac"
	DefaultBondDenom = "ubcvstake"
	DefaultBCVDemon  = "ubcv"
...
var (
	AccAddrBurn    = AccAddress(crypto.AddressHash([]byte("burnaddr")))

	//购买stakebcv销毁地址
	AccAddrBcvBurnFromBuyBcvStake = AccAddress(crypto.AddressHash([]byte("bcvburnfrombuybcvstakeaddr")))

	//存储创世bcvstake;验证人把BCV打到该地址，该地址会1:1把bcvstake给验证人
	//普通用户向销毁地址转移1个BCV;该地址向用户地址转移1.3个BCVSTAKE
	AccAddrBcvstakePool = AccAddress(crypto.AddressHash([]byte("bcvstakepooladdr")))

	//蓄电地址,存储用户的BCV,同时分配给用户能量
	AccAddrEnergyPool = AccAddress(crypto.AddressHash([]byte("energypooladdr")))

	//提取BAC的时候需要交电费,交的电费直接到该地址销毁
	AccAddrEnergyBurn = AccAddress(crypto.AddressHash([]byte("energyburnaddr")))

	RateBurnBcvToBcvstake = "1.3" //burn ubcv获取ubcvstake的比例 1.3
	RateBurnBcvToEnergy = "10" // 1ubcv energy
)
