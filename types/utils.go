...
/**
  * [0,1,2,3]
 */
func ParseTypeToUint8(flagStr string) (uint8, error) {
	flag := strings.TrimSpace(flagStr)
	if len(flag) == 0 {
		return 0, nil
	}
	sendType, err := strconv.Atoi(flag)
	if err != nil {
		return 0, err
	}
	if(sendType <0 || sendType>2){
		return 0,fmt.Errorf("integer out of range: %s", flagStr)
	}
	sendTypeCon := uint8(sendType)

	return sendTypeCon, nil
}