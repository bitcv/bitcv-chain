...
func TestBacAppGenTx(t *testing.T) {
	cdc := MakeCodec()
	_ = cdc
...
func TestBacAppGenState(t *testing.T) {
	cdc := MakeCodec()
	_ = cdc
	var genDoc tmtypes.GenesisDoc

	// test unmarshalling error
	_, err := BacAppGenState(cdc, genDoc, []json.RawMessage{})
	require.Error(t, err)
...
func TestBacGenesisValidation(t *testing.T) {
	...
	err := BacValidateGenesisState(genesisState)
	require.Error(t, err)
	...
	genesisState = makeGenesisState(t, genTxs)
	val1 := staking.NewValidator(valAddr1, pk1, staking.NewDescription("test #2", "", "", ""))
	val1.Jailed = true
	...
	val1.Jailed = false
	genesisState = makeGenesisState(t, genTxs)
	val2 := staking.NewValidator(valAddr1, pk1, staking.NewDescription("test #3", "", "", ""))
	genesisState.StakingData.Validators = append(genesisState.StakingData.Validators, val1)
...
func TestGenesisStateSanitize(t *testing.T) {
	...
	require.Nil(t, BacValidateGenesisState(genesisState))
