...
import (
	...
	authsim "github.com/bitcv-chain/bitcv-chain/x/auth/simulation"
	"github.com/bitcv-chain/bitcv-chain/x/bank"
	banksim "github.com/bitcv-chain/bitcv-chain/x/bank/simulation"
	distr "github.com/bitcv-chain/bitcv-chain/x/distribution"
	distrsim "github.com/bitcv-chain/bitcv-chain/x/distribution/simulation"
	"github.com/bitcv-chain/bitcv-chain/x/gov"
	govsim "github.com/bitcv-chain/bitcv-chain/x/gov/simulation"
	"github.com/bitcv-chain/bitcv-chain/x/mint"
	"github.com/bitcv-chain/bitcv-chain/x/simulation"
	"github.com/bitcv-chain/bitcv-chain/x/slashing"
	slashingsim "github.com/bitcv-chain/bitcv-chain/x/slashing/simulation"
	"github.com/bitcv-chain/bitcv-chain/x/staking"
	stakingsim "github.com/bitcv-chain/bitcv-chain/x/staking/simulation"
)
...
func getSimulateFromSeedInput(tb testing.TB, app *BacApp) (
...
func appStateRandomizedFn(r *rand.Rand, accs []simulation.Account, genesisTimestamp time.Time) (json.RawMessage, []simulation.Account, string) {
	...
	bankGenesis := bank.NewGenesisState(r.Int63n(2) == 0,nil)
...
func testAndRunTxs(app *BacApp) []simulation.WeightedOperation {
...
func invariants(app *BacApp) []sdk.Invariant {
...
// Profile with:
// /usr/local/go/bin/go test -benchmem -run=^$ github.com/bitcv-chain/bitcv-chain/app -bench ^BenchmarkFullBacSimulation$ -SimulationCommit=true -cpuprofile cpu.out
func BenchmarkFullBacSimulation(b *testing.B) {
	// Setup Bac application
	logger := log.NewNopLogger()

	var db dbm.DB
	dir, _ := ioutil.TempDir("", "goleveldb-bac-sim")
	db, _ = sdk.NewLevelDB("Simulation", dir)
	defer func() {
		db.Close()
		os.RemoveAll(dir)
	}()
	app := NewBacApp(logger, db, nil, true, 0)
...
func TestFullBacSimulation(t *testing.T) {
	if !enabled {
		t.Skip("Skipping Bac simulation")
	}
	...
	app := NewBacApp(logger, db, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "bacchain", app.Name())
...
func TestBacImportExport(t *testing.T) {
	if !enabled {
		t.Skip("Skipping Bac import/export simulation")
	}
	...
	newDir, _ := ioutil.TempDir("", "goleveldb-bac-sim-2")
	newDB, _ := sdk.NewLevelDB("Simulation-2", dir)
	defer func() {
		newDB.Close()
		os.RemoveAll(newDir)
	}()
	newApp := NewBacApp(log.NewNopLogger(), newDB, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "bacchain", newApp.Name())
...
func TestBacSimulationAfterImport(t *testing.T) {
	if !enabled {
		t.Skip("Skipping Bac simulation after import")
	}
