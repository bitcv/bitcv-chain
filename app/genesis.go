...
type GenesisState struct {
	...
	AccountsEdatas []GenesisAccountEdatas	`json:"account_edatas"`
...
func NewGenesisState(accounts []GenesisAccount, accountEdatas []GenesisAccountEdatas, authData auth.GenesisState,
...
type GenesisAccountEdatas struct {
	Address sdk.AccAddress `json:"address"`
	Edatas  []bank.Edata   `json:"edatas"`
}
...
// Create the core parameters for genesis initialization for bac
// note that the pubkey input is this machines pubkey
func BacAppGenState(cdc *codec.Codec, genDoc tmtypes.GenesisDoc, appGenTxs []json.RawMessage) (
...
// NewDefaultGenesisState generates the default state for bac.
...
// BacValidateGenesisState ensures that the genesis state obeys the expected invariants
// TODO: No validators are both bonded and jailed (#2088)
// TODO: Error if there is a duplicate validator (#1708)
// TODO: Ensure all state machine parameters are in genesis (#1704)
func BacValidateGenesisState(genesisState GenesisState) error {
...
// BacAppGenState but with JSON
func BacAppGenStateJSON(cdc *codec.Codec, genDoc tmtypes.GenesisDoc, appGenTxs []json.RawMessage) (
	appState json.RawMessage, err error) {
	// create the final app state
	genesisState, err := BacAppGenState(cdc, genDoc, appGenTxs)
	if err != nil {
		return nil, err
	}
