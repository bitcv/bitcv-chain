...
func TestCreateLedgerUnsupportedAlgo(t *testing.T) {
	kb := NewInMemory()
	_, err := kb.CreateLedger("some_account", Ed25519, "bac", 0, 1)
	assert.Error(t, err)
...
func TestCreateLedger(t *testing.T) {
	...
	ledger, err := kb.CreateLedger("some_account", Secp256k1, "bac", 3, 1)
	...
	assert.Equal(t, "bacpub1addwnpepqdszcr95mrqqs8lw099aa9h8h906zmet22pmwe9vquzcgvnm93eqygufdlv", pk)
...
func TestKeyManagement(t *testing.T) {
	...
	require.NoError(t, err)
	addr, err := sdk.AccAddressFromBech32("bac10tyhju9pfpfkt7hrd2zqr0vjn4k5sfrrhenf7v")
	require.NoError(t, err)
...
func TestAbc(t *testing.T)  {
	addr := "bac1a53pdvjcc5mu2s9ghmdthdlrydc953js6h69wx"
	b,err:= sdk.AccAddressFromBech32(addr)
	c  := sdk.ValAddress(b).String()
	fmt.Println(b)
	fmt.Println(err)
	fmt.Println(c)

	fmt.Println("1",sdk.AccAddrBcvstakePool.String())
}