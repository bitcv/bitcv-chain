...
import (
	...
	"github.com/bitcv-chain/bitcv-chain/server/config"
...
func interceptLoadConfig() (conf *cfg.Config, err error) {
	...
	// create a default bac config file if it does not exist
	bacConfigFilePath := filepath.Join(rootDir, "config/bacd.toml")
	if _, err := os.Stat(bacConfigFilePath); os.IsNotExist(err) {
		bacConf, _ := config.ParseConfig()
		config.WriteConfigFile(bacConfigFilePath, bacConf)
	}

	viper.SetConfigName("bacd")
