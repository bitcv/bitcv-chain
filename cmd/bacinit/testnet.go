...
import (
	...
	srvconfig "github.com/bitcv-chain/bitcv-chain/server/config"
	sdk "github.com/bitcv-chain/bitcv-chain/types"
	"github.com/bitcv-chain/bitcv-chain/x/auth"
	authtx "github.com/bitcv-chain/bitcv-chain/x/auth/client/txbuilder"
...
func TestnetFilesCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a Bacd testnet",
...
Example:
	bacd testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "bacd",
		"Home directory of the node's daemon configuration",
	)
	cmd.Flags().String(flagNodeCliHome, "baccli",
...
func initTestnet(config *tmconfig.Config, cdc *codec.Codec) error {
	...
	bacConfig := srvconfig.DefaultConfig()
	bacConfig.MinGasPrices = viper.GetString(server.FlagMinGasPrices)
	...
	for i := 0; i < numValidators; i++ {
		...
		bacConfigFilePath := filepath.Join(nodeDir, "config/bacd.toml")
		srvconfig.WriteConfigFile(bacConfigFilePath, bacConfig)
	}
