...
import (
	...
	kbkeys "github.com/bitcv-chain/bitcv-chain/crypto/keys"
	...
	"github.com/bitcv-chain/bitcv-chain/x/staking/client/cli"
)
...
// GenTxCmd builds the bacd gentx command.
// nolint: errcheck
func GenTxCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		Args:  cobra.NoArgs,
		Long: fmt.Sprintf(`This command is an alias of the 'bacd tx create-validator' command'.
...
`, defaultAmount, defaultCommissionRate, defaultCommissionMaxRate, defaultCommissionMaxChangeRate, defaultMinSelfDelegation),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err = app.BacValidateGenesisState(genesisState); err != nil {
				return err
			}
			...
			if info.GetType() == kbkeys.TypeOffline || info.GetType() == kbkeys.TypeMulti {
				fmt.Println("Offline key passed in. Use `baccli tx sign` command to sign:")
