...
const (
	...
	DefaultGasLimit      = 2000000 //该交易的执行时使用gas的上限
	GasFlagAuto          = "auto"
...
func PostCommands(cmds ...*cobra.Command) []*cobra.Command {
	for _, c := range cmds {
		...
		c.Flags().String(FlagFees, "", "Fees to pay along with transaction; eg: 10ubac")
		c.Flags().String(FlagGasPrices, "", "Gas prices to determine the transaction fee (e.g. 10ubac)")
		c.Flags().String(FlagNode, "tcp://localhost:26657", "<host>:<port> to tendermint rpc interface for this chain")
		c.Flags().Bool(FlagUseLedger, false, "Use a connected Ledger device")
